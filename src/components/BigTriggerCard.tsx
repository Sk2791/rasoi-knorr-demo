...
interface BigTriggerCardProps {
  ...
  // ARBITER's occasion-card summary + confidence scores — same data already
  // shown in Technical View, surfaced here too so the brand manager sees the
  // event summary and its quantified impact without switching views.
...
export const BigTriggerCard: React.FC<BigTriggerCardProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Occasion Card — ARBITER's summary + confidence scores, once it has run */}
      {occasionQuote !== "—" && (
        <div
          id="brand-occasion-card"
          className="border border-orange-500/30 rounded-2xl p-4 mb-4 bg-gradient-to-br from-orange-600/10 to-orange-800/10 relative z-10"
        >
          <h4 className="text-[10px] tracking-[0.15em] text-orange-400 uppercase font-bold mb-1.5">
            Occasion card · auto-generated by ARBITER
          </h4>
          <div className="text-sm font-bold leading-snug text-white mb-3">{occasionQuote}</div>
          <div className="grid grid-cols-3 gap-2.5 max-w-md">
            <div className="text-center border border-slate-800 rounded-xl p-2 bg-slate-900/80">
              <b className="block font-mono text-lg text-orange-400">{occasionScores[0]}</b>
              <span className="text-[9px] text-slate-400 tracking-wider uppercase">Occasion Fit</span>
            </div>
            <div className="text-center border border-slate-800 rounded-xl p-2 bg-slate-900/80">
              <b className="block font-mono text-lg text-orange-400">{occasionScores[1]}</b>
              <span className="text-[9px] text-slate-400 tracking-wider uppercase">Velocity</span>
            </div>
            <div className="text-center border border-slate-800 rounded-xl p-2 bg-slate-900/80">
              <b className="block font-mono text-lg text-orange-400">{occasionScores[2]}</b>
              <span className="text-[9px] text-slate-400 tracking-wider uppercase">Risk</span>
            </div>
          </div>
        </div>
      )}
